...
use fsh_lint_core::{
    CompiledRule, Diagnostic, FshLintError, GritQLMatcher, Result, Rule,
...
impl RuleRegistry {
    ...
    fn validate_pack_metadata(&self, metadata: &RulePackMetadata) -> Result<()> {
        if metadata.name.trim().is_empty() {
            return Err(FshLintError::config_error("Rule pack name cannot be empty"));
        }

        if metadata.version.trim().is_empty() {
            return Err(FshLintError::config_error(
                "Rule pack version cannot be empty",
            ));
        }
...
impl DefaultRuleEngine {
    ...
    pub fn load_rule_file(&mut self, path: &Path) -> Result<()> {
        ...
        let rule: Rule = if path.extension().and_then(|s| s.to_str()) == Some("json") {
            serde_json::from_str(&content).map_err(|e| {
                FshLintError::config_error(format!(
    ...
    fn load_rules_from_dir_flat(&mut self, dir: &Path, config: &RuleDiscoveryConfig) -> Result<()> {
        ...
        let entries = fs::read_dir(dir).map_err(|e| FshLintError::io_error(dir, e))?;

        for entry in entries {
            let entry = entry.map_err(|e| FshLintError::io_error(dir, e))?;
            let path = entry.path();
...
mod tests {
    use super::*;
    use fsh_lint_core::{RuleCategory, RuleMetadata, Severity};
    use std::fs;

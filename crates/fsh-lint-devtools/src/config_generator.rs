...
//!
//! Generates default fsh-lint configuration files in various formats.
...
mod tests {
    ...
    fn test_generate_default_json() {
        ...
        let output_path = temp_dir.path().join("fsh-lint.json");
    ...
    fn test_generate_default_toml() {
        let temp_dir = TempDir::new().unwrap();
        let output_path = temp_dir.path().join("fsh-lint.toml");

//! JSON Schema generator for fsh-lint configuration
//!
//! Generates JSON Schema from the Rust configuration types using schemars.

use anyhow::Result;
use fsh_lint_core::config::FshLintConfiguration;
...
impl SchemaGenerator {
    ...
    ///
    /// Generates a complete JSON Schema from the `FshLintConfiguration` type
    /// with metadata and writes it to the specified path.
    pub fn generate(output_path: &Path) -> Result<()> {
        tracing::info!("Generating JSON Schema for fsh-lint configuration...");

        // Generate schema from Rust types using schemars
        let schema = schema_for!(FshLintConfiguration);

        // Convert to JSON value for manipulation
        let mut schema_json = serde_json::to_value(schema)?;

        // Add metadata
        schema_json["$id"] = json!("https://octofhir.github.io/fsh-lint-rs/schema/v1.json");
        schema_json["$schema"] = json!("http://json-schema.org/draft-07/schema#");
        schema_json["title"] = json!("FSH Lint Configuration");
        schema_json["description"] = json!(
            "Configuration file schema for fsh-lint - validates fsh-lint.json and fsh-lint.jsonc files"
        );

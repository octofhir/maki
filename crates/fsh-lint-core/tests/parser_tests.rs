...
use fsh_lint_core::parser::{CachedFshParser, FshParser, ParseError, Parser, ParserConfig};
use std::io::Write;
use std::sync::Arc;
...
fn test_parser_creation() {
    ...
    assert!(
        test_parser.is_ok(),
        "Should be able to create test FSH parser"
    );
    ...
    match FshParser::new() {
        Ok(parser) => {
            ...
            assert!(
                language.version() > 0,
                "Real parser language should have a version"
            );
        }
        Err(_) => {
...
fn test_cached_parser_creation() {
    ...
    assert!(
        parser_with_config.is_ok(),
        "Should be able to create cached parser with config"
    );
}
...
fn test_parse_empty_content() {
    ...
    // Empty content is not valid FSH, so we don't assert `is_valid` here.
}
...
fn test_parse_whitespace_only() {
    ...
    assert!(
        result.is_ok(),
        "Should be able to parse whitespace-only content"
    );
    let parse_result = result.unwrap();
...
fn test_parse_valid_extension() {
    ...
    assert!(
        result.is_ok(),
        "Should be able to parse extension definition"
    );
    let parse_result = result.unwrap();
...
fn test_parse_multiple_resources() {
    ...
    assert!(
        !root.is_error(),
        "Multiple resources should parse without errors"
    );
}
...
fn test_parse_syntax_error() {
    ...
    // With the real grammar, syntax errors should be detected
...
fn test_parse_incomplete_object() {
    ...
    assert!(
        result.is_ok(),
        "Parser should return result even with incomplete object"
    );
    let parse_result = result.unwrap();

    // Should have parse errors for an incomplete definition
    if !parse_result.is_valid {
        assert!(
            !parse_result.errors().is_empty(),
            "Should have parse errors for incomplete object"
        );
    }
}
...
fn test_parse_mixed_valid_invalid() {
    ...
    assert!(
        result.is_ok(),
        "Parser should return result for mixed content"
    );
    let parse_result = result.unwrap();
...
fn test_parse_error_recovery() {
    ...
    let content_with_errors = r#"Profile: ValidProfileOne
Parent: Patient

Profile BrokenProfile
Parent: Patient

Profile: ValidProfileTwo
Parent: Patient
"#;
    ...
    assert!(
        result.is_ok(),
        "Parser should handle mixed valid/invalid content"
    );
    ...
    if !parse_result.is_valid {
        ...
        for error in errors {
            assert!(
                error.offset() < content_with_errors.len(),
                "Error offset should be within content"
            );
        }
    }
}
...
fn test_incremental_parsing() {
    ...
    let updated_content = r#"Profile: MyPatientProfile
Parent: Patient
...
"#;
    ...
    assert!(
        ...
        "Incremental parsing should work"
    );
    ...
    assert!(
        !root.is_error(),
        "Incrementally parsed content should be valid"
    );
}
...
fn test_incremental_parsing_with_errors() {
    ...
    let incremental_result =
        parser.parse_incremental(updated_content_with_error, &initial_result.tree);

    assert!(
        incremental_result.is_ok(),
        "Incremental parsing should work even with errors"
    );
    let parse_result = incremental_result.unwrap();
...
fn test_content_hash_consistency() {
    ...
    assert_eq!(
        hash1.size(),
        content1.len(),
        "Hash should track content size"
    );
}
...
fn test_cached_parser_basic_caching() {
    ...
    assert_eq!(
        stats_after_first.size, 1,
        "Cache should have one entry after first parse"
    );
    ...
    assert!(
        ...
        "Cached results should be the same Arc"
    );
...
fn test_cached_parser_different_content() {
    ...
    let content1 = "Profile: CacheA\nParent: Patient\n";
    let content2 = "Profile: CacheB\nParent: Patient\n";
    ...
    assert_eq!(
        stats.size, 2,
        "Cache should have two entries for different content"
    );
...
fn test_cached_parser_cache_invalidation() {
    let mut parser = CachedFshParser::new().unwrap();
    let content = "Profile: CacheInvalidate\nParent: Patient\n";
    ...
    assert_eq!(
        parser.cache_stats().size,
        1,
        "Cache should have one entry after re-parsing"
    );
...
fn test_cached_parser_incremental_with_cache() {
    ...
    let initial_content = "Profile: CacheInitial\nParent: Patient\n";
    let updated_content = "Profile: CacheInitial\nParent: Patient\n* name 1..1\n";
    ...
    let incremental_result = parser
        .parse_incremental_with_cache(&updated_content, &initial_result.tree)
        .unwrap();
    assert_eq!(
        parser.cache_stats().size,
        2,
        "Should cache both initial and updated content"
    );

    // Parse updated content again - should use cache
    let cached_result = parser.parse_with_cache(&updated_content).unwrap();
    assert!(
        Arc::ptr_eq(&incremental_result, &cached_result),
        "Should return cached incremental result"
    );
}
...
fn test_cached_parser_trait_implementation() {
    ...
    let updated_content = "Profile: TraitTest\nParent: Patient\n* name 1..1\n";
    let incremental_result = parser.parse_incremental(&updated_content, &initial_result.tree);
    assert!(
        incremental_result.is_ok(),
        "Parser trait incremental parse should work"
    );
...
fn test_cached_parser_language_change_clears_cache() {
    let mut parser = CachedFshParser::new().unwrap();
    let content = "Profile: LanguageChange\nParent: Patient\n";
    ...
    assert_eq!(
        parser.cache_stats().size,
        0,
        "Cache should be cleared after language change"
    );
}
...
fn test_parser_config_options() {
    ...
    assert!(
        default_config.enable_cache,
        "Cache should be enabled by default"
    );
    assert_eq!(
        default_config.cache_capacity, 1000,
        "Default cache capacity should be 1000"
    );
    ...
    assert!(
        !custom_config.enable_cache,
        "Custom config should respect enable_cache setting"
    );
    assert_eq!(
        custom_config.cache_capacity, 100,
        "Custom config should respect cache_capacity setting"
    );
    ...
    assert!(
        parser_result.is_ok(),
        "Should be able to create parser with custom config"
    );
}
...
fn test_parse_error_methods() {
    let error = ParseError::new("Test error".to_string(), 5, 10, 100, 20);
...
fn test_parse_large_content() {
    ...
    // Create large FSH content by repeating definitions
...
fn test_concurrent_parsing() {
    ...
    let content = "Profile: Concurrent\nParent: Patient\n";
...
fn test_cached_parser_concurrent_access() {
    ...
    let content = Arc::new("Profile: ConcurrentCached\nParent: Patient\n".to_string());
...
fn test_parser_error_handling() {
    ...
    let huge_content = "Profile: LargeProfile\nParent: Patient\n\n".repeat(5000);

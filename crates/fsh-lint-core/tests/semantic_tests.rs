...
use fsh_lint_core::{
    DefaultSemanticAnalyzer, SemanticAnalyzer, SemanticAnalyzerConfig,
    SemanticModel, FhirResource, ResourceType, SymbolType, ReferenceType,
    Cardinality, ElementFlag, Severity, Element, Location, ResourceMetadata,
    Reference, Symbol, TypeInfo, Constraint, ConstraintType
};
...
fn test_semantic_model_creation() {
    ...
    assert_eq!(profile.metadata.title, Some("My Patient Profile".to_string()));
    assert_eq!(profile.metadata.description, Some("A custom patient profile".to_string()));
    ...
    assert_eq!(name_element.cardinality, Some(Cardinality { min: 1, max: Some(1) }));
    assert!(name_element.flags.contains(&ElementFlag::MustSupport));
    
    // Check extension
    let extension = model.get_resource("MyExtension").expect("Extension not found");
...
fn test_element_validation() {
    ...
    assert_eq!(element.cardinality, Some(Cardinality { min: 1, max: Some(1) }));
...
fn test_symbol_table_building() {
    ...
    // Check symbol types
    let patient_symbol = model.symbols().get_symbol("MyPatient").expect("MyPatient symbol not found");
    assert_eq!(patient_symbol.symbol_type, SymbolType::Profile);
    
    let extension_symbol = model.symbols().get_symbol("MyExtension").expect("MyExtension symbol not found");
...
fn test_reference_creation() {
    ...
    let parent_refs: Vec<_> = model.references.iter()
...
fn test_cardinality_parsing() {
    let test_cases = vec![
        ("1..1", Some(Cardinality { min: 1, max: Some(1) })),
        ("0..1", Some(Cardinality { min: 0, max: Some(1) })),
        ("1..*", Some(Cardinality { min: 1, max: None })),
        ("0..*", Some(Cardinality { min: 0, max: None })),
        ("5..10", Some(Cardinality { min: 5, max: Some(10) })),
    ];
    
    let analyzer = DefaultSemanticAnalyzer::new();
    
    for (cardinality_str, expected) in test_cases {
        let result = analyzer.parse_cardinality(cardinality_str).expect("Failed to parse cardinality");
        assert_eq!(result, expected, "Failed for cardinality: {}", cardinality_str);
    }
}
...
fn test_reference_target_extraction() {
    ...
    let test_cases = vec![
        ...
        ("Reference(Patient | Practitioner)", vec!["Patient".to_string(), "Practitioner".to_string()]),
        ("Reference(Patient|Practitioner|Organization)", vec!["Patient".to_string(), "Practitioner".to_string(), "Organization".to_string()]),
        ("string", vec![]),
    ];
    
    for (type_name, expected) in test_cases {
        let result = analyzer.extract_reference_targets(type_name).expect("Failed to extract reference targets");
...
fn test_semantic_validation_unresolved_references() {
    ...
    let unresolved_errors: Vec<_> = diagnostics.iter()
...
fn test_semantic_validation_missing_metadata() {
    ...
    let missing_title: Vec<_> = diagnostics.iter()
        .filter(|d| d.rule_id == "missing-title")
        .collect();
    let missing_description: Vec<_> = diagnostics.iter()
...
fn test_semantic_validation_invalid_cardinality() {
    ...
    let profile_with_invalid_cardinality = FhirResource {
        ...
        elements: vec![
            Element {
                path: "name".to_string(),
                cardinality: Some(Cardinality { min: 5, max: Some(1) }), // Invalid: min > max
                type_info: None,
                constraints: Vec::new(),
                location: test_location(),
                flags: Vec::new(),
    ...
    let cardinality_errors: Vec<_> = diagnostics.iter()
...
fn test_semantic_validation_profile_missing_parent() {
    ...
    let parent_errors: Vec<_> = diagnostics.iter()
...
fn test_resource_id_validation() {
    ...
    let invalid_ids = vec!["", "123Invalid", "-InvalidStart", "Invalid.ID", "Invalid ID"];
    ...
    for id in invalid_ids {
        assert!(!analyzer.is_valid_resource_id(id), "Should be invalid: {}", id);
    }
}
...
fn test_element_path_validation() {
    ...
    let valid_paths = vec!["name", "name.given", "contact.telecom.value", "extension.value"];
    let invalid_paths = vec!["", ".name", "name.", "name..given", "name."];
    
    for path in valid_paths {
        assert!(analyzer.is_valid_element_path(path), "Should be valid: {}", path);
    }
    
    for path in invalid_paths {
        assert!(!analyzer.is_valid_element_path(path), "Should be invalid: {}", path);
    }
}
...
fn test_fhir_type_validation() {
    ...
    let valid_types = vec![
        "string", "boolean", "integer", "Patient", "Practitioner",
        "Reference(Patient)", "Reference(Patient | Practitioner)"
    ];
    
    for type_name in valid_types {
        assert!(analyzer.is_valid_fhir_type(type_name), "Should be valid: {}", type_name);
    }
}
...
fn test_semantic_model_resource_queries() {
    ...
    assert_eq!(patient_resource.unwrap().resource_type, ResourceType::Profile);
...
fn test_reference_resolution() {
    ...
    // Resolve references
    analyzer.resolve_references(&mut model).expect("Failed to resolve references");
...
fn test_constraint_handling() {
    ...
    assert!(element.constraints.iter().any(|c| c.constraint_type == ConstraintType::FixedValue));
    assert!(element.constraints.iter().any(|c| c.constraint_type == ConstraintType::Binding));

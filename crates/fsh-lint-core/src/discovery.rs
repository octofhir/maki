            ...
            .filter_map(|e| e.ok())
        {
            ...
            if path.is_file() {
                if let Some(ext) = path.extension() {
                    ...
                    if extensions.iter().any(|allowed_ext| allowed_ext.to_lowercase() == ext_str) {
    ...
    fn discover_files(&self, config: &Config) -> Result<Vec<PathBuf>> {
        ...
        if !config.exclude_patterns.is_empty() {
            let exclude_patterns: Result<Vec<Pattern>> = config
                .exclude_patterns
                .iter()
                .map(|p| Pattern::new(p).map_err(|e| FshLintError::ConfigError {
            ...
            discovered_files.retain(|path| {
                !self.is_ignored(path, &exclude_patterns)
            });
        }
...
impl FileWatcher {
    ...
    pub fn filter_by_extension(events: Vec<FileChangeEvent>, extensions: &[&str]) -> Vec<FileChangeEvent> {
        events.into_iter()
            .filter(|event| {
                if let Some(ext) = event.path.extension() {
                    let ext_str = ext.to_string_lossy().to_lowercase();
                    extensions.iter().any(|&allowed_ext| allowed_ext.to_lowercase() == ext_str)
                } else {
                    false
                }
            })
...
mod tests {
    ...
    fn test_file_change_event_filtering() {
        ...
        assert!(fsh_events.iter().all(|e| e.path.extension().unwrap() == "fsh"));

        // Test FSH-specific filtering
        let fsh_only = FileWatcher::filter_fsh_files(events);
        assert_eq!(fsh_only.len(), 2);
        assert!(fsh_only.iter().all(|e| e.path.extension().unwrap() == "fsh"));
    }
    ...
    fn test_glob_pattern_matching() {
        ...
        fs::write(root.join("src/extensions/address.fsh"), "Extension: Address").unwrap();
        fs::write(root.join("test/fixtures/sample.fsh"), "Profile: Sample").unwrap();
        fs::write(root.join("build/output/generated.fsh"), "Profile: Generated").unwrap();
        fs::write(root.join("readme.md"), "# README").unwrap();
    ...
    fn test_ignore_file_handling() {
        ...
        assert!(files.iter().any(|f| f.file_name().unwrap() == "important.fsh"));
    ...
    fn test_complex_file_structure() {
        ...
        let files = [
            ("project/src/main/fsh/profiles/patient.fsh", "Profile: Patient"),
            ("project/src/main/fsh/extensions/address.fsh", "Extension: Address"),
            ("project/src/test/fsh/examples/example.fsh", "Instance: Example"),

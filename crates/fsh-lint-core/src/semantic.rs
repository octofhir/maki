...
use crate::{Diagnostic, Location, Result, FshLintError};
...
impl SymbolTable {
    ...
    pub fn add_symbol(&mut self, symbol: Symbol) {
        ...
        self.file_symbols.entry(file_path).or_insert_with(Vec::new).push(symbol_name);
    }
    ...
    pub fn get_symbols_in_file(&self, file_path: &PathBuf) -> Vec<&Symbol> {
        if let Some(symbol_names) = self.file_symbols.get(file_path) {
            symbol_names.iter().filter_map(|name| self.symbols.get(name)).collect()
        } else {
            Vec::new()
        }
    }
...
impl DefaultSemanticAnalyzer {
    ...
    fn extract_resources(&self, root_node: Node, source: &str, file_path: &PathBuf) -> Result<Vec<FhirResource>> {
    ...
    fn extract_resource_from_node(&self, node: Node, source: &str, file_path: &PathBuf) -> Result<Option<FhirResource>> {
    ...
    fn extract_resource_id(&self, node: Node, source: &str) -> Result<String> {
        ...
        if cursor.goto_first_child() {
            loop {
                ...
                if child.kind() == "identifier" {
                    return Ok(child.utf8_text(source.as_bytes())
                        .map_err(|e| FshLintError::semantic_error(format!("Failed to extract resource ID: {}", e)))?
                        .to_string());
                }
        ...
        Err(FshLintError::semantic_error("Resource ID not found".to_string()))
    }
                    ...
                    if parent_cursor.goto_first_child() {
                        loop {
                            ...
                            if parent_child.kind() == "identifier" {
                                return Ok(Some(parent_child.utf8_text(source.as_bytes())
                                    .map_err(|e| FshLintError::semantic_error(format!("Failed to extract parent: {}", e)))?
                                    .to_string()));
                            }
    ...
    fn extract_elements(&self, node: Node, source: &str, file_path: &PathBuf) -> Result<Vec<Element>> {
    ...
    fn extract_element_from_node(&self, node: Node, source: &str, file_path: &PathBuf) -> Result<Option<Element>> {
        let node_type = node.kind();
        ...
        if cursor.goto_first_child() {
            loop {
                ...
                if child.kind() == "path" || child.kind() == "element_path" {
                    return Ok(Some(child.utf8_text(source.as_bytes())
                        .map_err(|e| FshLintError::semantic_error(format!("Failed to extract element path: {}", e)))?
                        .to_string()));
                }
        ...
        if cursor.goto_first_child() {
            loop {
                ...
                if child.kind() == "cardinality" {
                    ...
                    let cardinality_text = child.utf8_text(source.as_bytes())
                        .map_err(|e| FshLintError::semantic_error(format!("Failed to extract cardinality: {}", e)))?;
    ...
    pub fn parse_cardinality(&self, cardinality_str: &str) -> Result<Option<Cardinality>> {
        if let Some((min_str, max_str)) = cardinality_str.split_once("..") {
            let min = min_str.parse::<u32>()
                .map_err(|e| FshLintError::semantic_error(format!("Invalid cardinality min: {}", e)))?;
            
            let max = if max_str == "*" {
                None
            } else {
                Some(max_str.parse::<u32>()
                    .map_err(|e| FshLintError::semantic_error(format!("Invalid cardinality max: {}", e)))?)
            };
        ...
        if cursor.goto_first_child() {
            loop {
                ...
                if child.kind() == "type" || child.kind() == "identifier" {
                    return Ok(Some(child.utf8_text(source.as_bytes())
                        .map_err(|e| FshLintError::semantic_error(format!("Failed to extract type name: {}", e)))?
                        .to_string()));
                }
    ...
    pub fn extract_reference_targets(&self, type_name: &str) -> Result<Vec<String>> {
        if let Some(targets_str) = type_name.strip_prefix("Reference(").and_then(|s| s.strip_suffix(")")) {
    ...
    fn extract_constraints(&self, node: Node, source: &str, file_path: &PathBuf) -> Result<Vec<Constraint>> {
    ...
    fn extract_constraint_value(&self, node: Node, source: &str) -> Result<Option<String>> {
        ...
        if cursor.goto_first_child() {
            loop {
                ...
                if child.kind() == "value" || child.kind() == "string" || child.kind() == "identifier" {
                    return Ok(Some(child.utf8_text(source.as_bytes())
                        .map_err(|e| FshLintError::semantic_error(format!("Failed to extract constraint value: {}", e)))?
                        .to_string()));
                }
            ...
            loop {
                ...
                if child.kind() == "string" || child.kind() == "value" {
                    let value = child.utf8_text(source.as_bytes())
                        .map_err(|e| FshLintError::semantic_error(format!("Failed to extract metadata value: {}", e)))?;
        ...
        for resource in &model.resources {
            if let Some(existing_location) = resource_ids.insert(&resource.id, &resource.location) {
                let diagnostic = Diagnostic::new(
                    ...
                    crate::Severity::Error,
                    format!("Duplicate resource ID '{}' (first defined at {})", resource.id, existing_location),
...
impl DefaultSemanticAnalyzer {
    ...
    fn validate_resource(&self, resource: &FhirResource) -> Vec<Diagnostic> {
            ...
            ResourceType::Profile | ResourceType::Extension => {
                if resource.title.is_none() && resource.metadata.title.is_none() {
                    let diagnostic = Diagnostic::new(
                        ...
                        format!("{} '{}' should have a title", resource.resource_type, resource.id),
                        resource.location.clone(),
                    );
                    diagnostics.push(diagnostic);
                }
                ...
                if resource.description.is_none() && resource.metadata.description.is_none() {
                    let diagnostic = Diagnostic::new(
                        "missing-description",
                        crate::Severity::Warning,
                        format!("{} '{}' should have a description", resource.resource_type, resource.id),
                        resource.location.clone(),
                    );
                    diagnostics.push(diagnostic);
                }
        ...
        if !self.is_valid_resource_id(&resource.id) {
            let diagnostic = Diagnostic::new(
                "invalid-resource-id",
                crate::Severity::Error,
                format!("Invalid resource ID '{}'. IDs must start with a letter and contain only letters, numbers, and hyphens", resource.id),
                resource.location.clone(),
            );
            diagnostics.push(diagnostic);
        }
    ...
    fn validate_element(&self, element: &Element, resource: &FhirResource) -> Vec<Diagnostic> {
            ...
            if let Some(max) = cardinality.max {
                if cardinality.min > max {
                    let diagnostic = Diagnostic::new(
                        "invalid-cardinality",
                        crate::Severity::Error,
                        format!("Invalid cardinality {}: minimum ({}) cannot be greater than maximum ({})", 
        ...
        if !self.is_valid_element_path(&element.path) {
            let diagnostic = Diagnostic::new(
                ...
                crate::Severity::Error,
                format!("Invalid element path '{}'. Paths should use dot notation (e.g., 'name.given')", element.path),
                element.location.clone(),
            );
            diagnostics.push(diagnostic);
        }
        ...
        let fixed_values: Vec<_> = element.constraints.iter()
        ...
        if fixed_values.len() > 1 {
            let diagnostic = Diagnostic::new(
                ...
                crate::Severity::Error,
                format!("Element '{}' has multiple fixed values, only one is allowed", element.path),
                element.location.clone(),
            );
            diagnostics.push(diagnostic);
        }
    ...
    pub fn is_valid_fhir_type(&self, type_name: &str) -> bool {
        ...
        const RESOURCE_TYPES: &[&str] = &[
            ...
            "DiagnosticReport", "DocumentReference", "Bundle", "OperationOutcome"
        ];

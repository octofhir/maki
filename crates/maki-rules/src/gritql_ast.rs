...
use maki_core::cst::ast::{AstNode, *};
use maki_core::{Diagnostic, Location, Severity};
...
fn evaluate_predicates(predicates: &[Predicate], context: &NodeContext) -> maki_core::Result<bool> {
...
fn evaluate_predicate(predicate: &Predicate, context: &NodeContext) -> maki_core::Result<bool> {
    match predicate {
...
fn get_field_value(field: &str, context: &NodeContext) -> maki_core::Result<Option<String>> {
    match context {

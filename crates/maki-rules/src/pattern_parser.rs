...
pub fn parse_pattern(pattern_text: &str) -> Result<AstPattern> {
    ...
    if pattern_text.is_empty() {
        return Err(MakiError::config_error("Pattern cannot be empty"));
    }
...
fn parse_node_type(type_str: &str) -> Result<NodeType> {
    match type_str {
        ...
        "*" | "Any" => Ok(NodeType::Any),
        other => Err(MakiError::config_error(format!(
...
fn parse_single_predicate(pred: &str) -> Result<Predicate> {
    ...
    Err(MakiError::config_error(format!(
...
fn extract_string_value(s: &str) -> Result<String> {
    ...
    if (s.starts_with('"') && s.ends_with('"')) || (s.starts_with('\'') && s.ends_with('\'')) {
        if s.len() < 2 {
            return Err(MakiError::config_error("Empty string value"));
        }

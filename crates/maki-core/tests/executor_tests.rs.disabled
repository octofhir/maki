...
use maki_core::parser::{ParseResult, Parser};
use maki_core::{
    CompiledRule, DefaultExecutor, Diagnostic, ExecutionContext, Executor, Location,
    MakiConfiguration, MakiError, ProgressCallback, ProgressInfo, ResourceStats, Result,
...
impl Parser for MockParser {
    fn parse(&mut self, content: &str) -> Result<ParseResult> {
        ...
        if self.should_fail {
            return Err(MakiError::ParseError {
...
impl SemanticAnalyzer for MockSemanticAnalyzer {
    fn analyze(
        ...
        if self.should_fail {
            return Err(MakiError::SemanticError {
...
impl RuleEngine for MockRuleEngine {
    ...
    fn compile_rule(&self, _rule: &maki_core::Rule) -> Result<CompiledRule> {
        // Return a mock compiled rule
        Ok(CompiledRule {
            metadata: maki_core::RuleMetadata {
                id: "mock/correctness/mock-rule".to_string(),
                name: "Mock Rule".to_string(),
                description: "A mock rule for testing".to_string(),
                severity: Severity::Warning,
                category: maki_core::RuleCategory::Correctness,
                tags: vec!["test".to_string()],
                version: None,
                docs_url: None,
            },
            matcher: maki_core::GritQLMatcher::new("mock pattern".to_string()).unwrap(),
    ...
    fn validate_rule(&self, _rule: &maki_core::Rule) -> Result<()> {
        Ok(())
    }
}
...
fn test_semantic_error_handling() {
    ...
    for result in results {
        assert!(result.error.is_some());
        match result.error.unwrap() {
            MakiError::SemanticError { .. } => {} // Expected

...
impl DefaultSemanticAnalyzer {
    ...
    pub fn parse_cardinality(&self, text: &str) -> Result<Option<Cardinality>> {
        ...
        if parts.len() != 2 {
            return Err(MakiError::semantic_error(format!(
                "Invalid cardinality expression: {text}"
            )));
        }

        let min = parts[0].parse::<u32>().map_err(|_| {
            MakiError::semantic_error(format!("Invalid minimum cardinality in {text}"))
        })?;
        ...
        let max = if parts[1] == "*" {
            ...
            Some(parts[1].parse::<u32>().map_err(|_| {
                MakiError::semantic_error(format!("Invalid maximum cardinality in {text}"))
            })?)
        };
...
impl SemanticAnalyzer for DefaultSemanticAnalyzer {
    fn analyze(
        ...
        let document = Document::cast(cst.clone())
            .ok_or_else(|| MakiError::semantic_error("Root node is not a Document".to_string()))?;

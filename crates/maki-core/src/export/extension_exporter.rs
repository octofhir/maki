...
impl ExtensionExporter {
    ...
    fn apply_metadata(
        ...
        structure_def.base_definition = Some("http://hl7.org/fhir/StructureDefinition/Extension".to_string());
    ...
    fn set_extension_context(
        ...
        let default_context = vec![
            StructureDefinitionContext::element("Element")
        ];
    ...
    fn ensure_extension_structure(
        ...
        if let Some(url_element) = snapshot.element.iter_mut().find(|e| e.path == "Extension.url") {
        ...
        let has_value_constraint = snapshot
            .element
            .iter()
            .any(|e| e.path.starts_with("Extension.value[x]") || e.path.starts_with("Extension.value"));
        ...
        if has_value_constraint {
            debug!("Extension {} is a simple extension (has value[x])", structure_def.name);
        } else if has_extension_constraint {
            debug!("Extension {} is a complex extension (has sub-extensions)", structure_def.name);
        }

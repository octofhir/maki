...
fn test_comprehensive_sushi_compliance() {
    ...
    println!("Total test cases: {}", harness.test_cases.len());
    ...
    let semantic_results: Vec<_> = results.iter()
...
fn test_regression_prevention() {
    ...
    if !results.is_empty() {
        ...
        let semantic_issues: Vec<_> = results.iter()
            .flat_map(|r| &r.semantic_results)
        ...
        if !semantic_issues.is_empty() {
            ...
            for result in semantic_issues {
                println!("  File: {} (Score: {:.2})", result.file, result.equivalence_score);
...
fn analyze_performance(results: &[sushi_compatibility::ComparisonResult], total_time: std::time::Duration) {
    println!("\nPerformance Analysis:");
    println!("===================");
...
fn generate_ci_artifacts(harness: &SushiCompatibilityHarness, results: &[sushi_compatibility::ComparisonResult]) {
    if let Ok(reports_dir) = env::var("MAKI_REPORTS_DIR") {

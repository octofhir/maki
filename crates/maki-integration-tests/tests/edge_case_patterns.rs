...
fn validate_results(harness: &SushiCompatibilityHarness, results: &[sushi_compatibility::ComparisonResult], test_type: &str) {
    if !results.is_empty() {
        let report = harness.generate_report(results);

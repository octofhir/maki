...
#[command(
    long_about = "fsh-lint is a fast, extensible linter and formatter for FHIR Shorthand (FSH) files.\n\
It provides comprehensive syntax checking, semantic validation, and automatic fixing capabilities.\n\
\n\
Examples:\n  \
fsh-lint lint                    # Lint current directory\n  \
fsh-lint lint --fix src/         # Lint and fix files in src/\n  \
fsh-lint fmt --check .           # Check formatting without changes\n  \
fsh-lint rules --verbose         # List all available rules\n  \
fsh-lint config init             # Initialize configuration file"
)]
struct Cli {
    #[command(subcommand)]
    ...
    #[arg(
        ...
        long,
        global = true,
        help = "Path to configuration file (.fshlintrc.json/.fshlintrc.toml)"
    )]
...
enum ConfigAction {
    ...
    Validate {
        ...
        #[arg(help = "Path to configuration file (default: search for .fshlintrc)")]
        path: Option<PathBuf>,
    },
...
async fn run_command(cli: Cli) -> Result<()> {
    match cli.command {
        ...
        Some(Commands::Version { detailed }) => {
            if detailed {
                println!("fsh-lint {}", fsh_lint_core::VERSION);
